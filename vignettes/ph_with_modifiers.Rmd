...
ext_img <- external_img(img_file, width = 1.39, height = 1.08)

anyplot <- plot_instr(code = barplot(1:5, col = 2:6))

read_pptx() |>
  add_slide("Two Content") |>
  ph_with(location = "body[1]", ext_img, use_loc_size = FALSE,
          rot = 10, bg = "#ff000015", ln = "red") |>
  add_slide("Two Content") |>
  ph_with(location = "body[1]", anyplot,
          rot = -5, bg = "#0000ff15", ln = "green") |>
  print(target = tempfile(fileext = ".pptx"))
```

```{r echo = FALSE, out.width = "100%"}
knitr::include_graphics("img_images_example.png")
```
